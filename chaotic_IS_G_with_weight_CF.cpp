// chaotic inflation に importance sampling を実行．全ての omega に対して Normal Distributionを変更．DNを使って、なだらかなノイズを追加
// さらに、weight と Compaction functionを出力する
...
const string filename_c = "Lattice_chaotic_biased_g_CF.dat"; // 出力ファイル名(曲率ゆらぎ)
const string filename_f = "field_chaotic_biased_g_CF.dat"; // 出力ファイル名(phi, pi)
const string filename_cf = "chaotic_weight.dat";
const double Nf = 5.5;  // lattice 終了時刻
const double dN = 0.02; // 時間刻み
...
vector<vector<vector<double>>> dwdNend;
...
int main()
{
  ...
  ofstream ofs_cf(filename_cf);
  ofs << setprecision(10);
  ofs_c << setprecision(10);
  ofs_f << setprecision(10);
  ofs_cf << setprecision(10);
  ...
  LOOP cout << de[i][j][k] / x[i][j][k][1] / x[i][j][k][1] / 3. << endl;
...
#pragma omp critical
    ...
    //------------weight------------
    //double lnw = lnweight(NCL,dwdNlist,bias);
    int step=Nf/dN;
    cout << dwdNlist[step][i][j][k]<<"   ";
